...
class CatchWrapper(object):
    ...
    def __call__(self, function):
        @wraps(function)
        async def async_wrapped(obj_self, data: BinanceData):
            try:
...
class RetryWrapper(object):
    def __call__(self, function):
        @wraps(function)
        async def async_wrapped(obj_self, data: BinanceData, url: str):
            tries = 1
...
class BinanceFuturesUMKlineUpdater(IKlineUpdater):
    ...
    def get_urls(self, data: BinanceData) -> BinanceData:
        """
        接受一个data对象
    ...
    @CatchWrapper()  # 捕获MaxTriesError/StatusError，并设置给data
    async def get_klines(self, data: BinanceData) -> BinanceData:
        # get raw
    ...
    @RetryWrapper()  # 捕获ClientError并重试；抛出MaxTriesError，由catch捕获
    @TimeoutWrapper()  # 抛出TimeoutError，由_with_retries捕获
    async def _request(self, data: BinanceData, url: str):
        """
        一般情况下，url出错会直接无法连接
        也有可能是404
        标的名称出错，返回是400
    ...
    async def __call__(self, data: BinanceData) -> BinanceData:
